...
Matrix::Matrix(int height, int width, float val, bool diagonal)
    : std::vector< std::vector<float> >( height, vector<float>( width, diagonal ? 0 : val))
{
...
Polynom::Polynom(int len, float val) : vector<float>(len, val)
{
}
...
float Polynom::getValue(float x)
{
    float res = 0;
    float px = 1;
...
float RationalFunction::getValue(float x)
{
